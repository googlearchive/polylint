...
function extractBadBindingExpression(text) {
  // ** Regex breakdown **
  // \{\{ - Match the binding expression opening
  // (?:  ...  )* - Match one or more of the following, with the first four "eating" anything in quotes:
  //     \\' - Single quote escaped with backslash
  //     '(?:\\'|[^'])*' - A string between two single quotes, ignoring escaped quotes
  //     \\" - Double quote escaped with backslash
  //     "(?:\\"|[^"])*" - A string between two double quotes, ignoring escaped quotes
  //     [^\}'"] - None of the above matched, so now match any other intermediate character
  // \}(?!\}) - Match a faulty binding expression ending
