...
var linters = {
  ...
  elementNotDefined: function elementNotDefined(analyzer) {
    ...
    console.log(Object.keys(analyzer.elementsByTagName));
    customElements.forEach(function(element){
      if (analyzer.elementsByTagName[element.tagName] === undefined) {
        if (element.tagName == "dom-module") {
          /** dom-module gets created without using Polymer's syntactic sugar, but we definitely don't want to warn if it doesn't exist */
          return;
        }
