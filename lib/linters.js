...
function lintErrorFromJavascript(htmlNode, javascriptNode, href, message) {
  console.log(href, javascriptNode);
...
var linters = {
  ...
  domModuleAfterPolymer: function domModuleAfterPolymer(analyzer, path) {
    ...
    analyzer.elements.forEach(function(element){
      ...
      var domModule = dom5.nodeWalkAllPrior(script, domModuleForId(element.is));
      if (domModule.length > 0 && domModule[0]) {
        return;
      }
      var otherDomModule = dom5.nodeWalkAll(loadedDoc, domModuleForId(element.is));
      if (otherDomModule.length > 0) {
        var originalDomModule = analyzer.nodeWalkDocuments(domModuleForId(element.is));
        unorderedModules.push(lintErrorFromNode(originalDomModule[0], "dom-module " +
          "for " + element.is +
