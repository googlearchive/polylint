    ...
    analyzer.elements.forEach(function(element){
      ...
      if (domModules.length > 1) {
        domModules.forEach(function(domModule) {
          unorderedModules.push(lintErrorFromNode(domModule, "dom-module " +
            "has a repeated value for id " + element.is + "."), true);
        });
        return;
      }
      ...
      if (!otherDomModule) {
        ...
        unorderedModules.push(lintErrorFromNode(originalDomModule, "dom-module " +
          "for " + element.is +
          " should be a parent of it or earlier in the document.", true));
      }
    });
  ...
  unbalancedDelimiters: function unbalancedDelimiters(analyzer) {
    ...
    analyzer.elements.forEach(function(element) {
      ...
      expressions.forEach(function(expression){
        ...
        undeclaredBindings.push(lintErrorFromNode(expression.node, message, true));
      });
    });
    return undeclaredBindings;
  },
  jsConformance: function (analyzer, path, options) {
    var jsconfPolicy = options.jsconfPolicy;
    var warnings = [];
    if (jsconfPolicy) {
      warnings = JsConf.lint(
        analyzer.parsedScripts,
        jsconfPolicy,
        {
          path: path,
