...
var linters = {
  ...
  styleBeyondTemplate: function styleBeyondTemplate(analyzer) {
    var styleLeaks = [];
    var isStyleLeak = p.AND(
      p.hasTagName('style'),
      p.NOT(parentIsTemplate)
    );
    analyzer.elements.forEach(function(element) {
      var domModules = getDomModules(analyzer, element.is);
      if (!domModules[0]) return;
      domModules.forEach(function(domModule) {
        var styles = dom5.nodeWalkAll(domModule, isStyleLeak);
        styles.forEach(function(style) {
          styleLeaks.push(lintErrorFromNode(style,
            'Style tag is not wrapped in template'));
        });
      });
    });
    return styleLeaks;
