  ...
  textNodes.forEach(function(textNode) {
    if (isBindingExpression(textNode.value)) {
      expressions.push({
        ...
        node: textNode,
        unwrapped: extractBindingExpression(textNode.value)
      });
    }
  });
  return expressions;
  ...
  attributes.forEach(function(attribute) {
    if (attribute.value && isBindingExpression(attribute.value)) {
      expressions.push({
        ...
        node: attribute.node,
        name: attribute.name,
        unwrapped: extractBindingExpression(attribute.value)
      });
    }
  });
  return expressions;
  ...
  boundVariablesDeclared: function findBindingToClass(analyzer) {
    ...
    analyzer.elements.forEach(function(element) {
      ...
      attributeExpressions.forEach(function(attributeExpression){
        ...
        var camelCase = caseMap.dashToCamelCase(attr.name);
        for (var i = 0; i < element.properties.length; i++) {
          var property = element.properties[i];
          if (property.name == camelCase) {
            if (!implicitExpressions[attributeExpression.unwrapped]) {
              implicitExpressions[attributeExpression.unwrapped] = [];
            }
            implicitExpressions[attributeExpression.unwrapped].push(attributeExpression);
          }
        }
      });
      ...
      expressions.forEach(function(expression){
        var unwrapped = expression.unwrapped;
        if (implicitBindings[unwrapped]) {
          return;
        }
